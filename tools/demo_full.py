...
def main():
    ...
    with torch.no_grad():
        for idx, data_dict in enumerate(demo_dataset):
            ...
            roi_boxes3d_corner = np.array([[35.2, 0, -1, 70.4, 80, 4, 0]])
            pc_data = demo_dataset.get_pc(idx)

            V.draw_scenes(
                points=pc_data[:, :-1], ref_boxes=pred_dicts[0]['pred_boxes'],
                ref_scores=pred_dicts[0]['pred_scores'], ref_labels=pred_dicts[0]['pred_labels'],
                gt_boxes=roi_boxes3d_corner
            )
